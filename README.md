...
建議依照以下結構整理您的檔案，這會讓您的 Repo 看起來極具專業工程規格：

```text
fintech-quantitative-trading-models/
│
├── .gitignore              (排除 Python 編譯暫存或敏感的 API 憑證)
├── README.md               (本說明文件)
│
├── src/                    (原始碼資料夾)
│   ├── data_fetcher.py     (股票數據抓取邏輯)
│   ├── backtest_strategy1.py (個股策略與 DCA/Lump-sum 回測)
│   └── pairs_trading.py    (配對交易與時序驗證核心程式)
│
└── docs/                   (專題報告與簡報)
    ├── AIFT_MidtermProject_Report.docx (包含研究結果、圖表與心得討論的 Word 檔)
    └── Demo.pptx                       (課堂簡報 PPT 檔案)